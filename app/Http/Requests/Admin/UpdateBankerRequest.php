...
class UpdateBankerRequest extends FormRequest
{
    /**
    ...
    {
        $banker = $this->route('banker');
        $bankerId = $banker ? $banker->id : null;

        return [
            'npc_id' => ['required', 'integer', 'exists:npcs,id', Rule::unique('bankers', 'npc_id')->ignore($bankerId)],
