...
@section('title', 'Редактировать диалог')

@section('content')
<div class="max-w-6xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl sm:text-3xl font-semibold mb-2">Редактировать диалог</h1>
        <a
    ...
    <form method="POST" action="{{ route('admin.dialogs.update', $dialog) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-[#161615] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg p-4 sm:p-6 space-y-4">
            ...
            <div>
                ...
                >
                    @foreach($npcs as $npc)
                        <option value="{{ $npc->id }}" {{ old('npc_id', $dialog->npc_id) == $npc->id ? 'selected' : '' }}>
                            {{ $npc->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    >
                        ...
                        @foreach($dialogs as $parentDialog)
                            <option value="{{ $parentDialog->id }}" {{ old('parent_dialog_id', $dialog->parent_dialog_id) == $parentDialog->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input
                        ...
                        id="min_level"
                        name="min_level"
                        value="{{ old('min_level', $dialog->min_level) }}"
                        min="1"
                        max="100"
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
                    >
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($quests as $quest)
                            <option value="{{ $quest->id }}" {{ old('required_quest_id', $dialog->required_quest_id) == $quest->id ? 'selected' : '' }}>
                                {{ $quest->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($questStatuses as $statusKey => $statusLabel)
                            <option value="{{ $statusKey }}" {{ old('required_quest_status', $dialog->required_quest_status) === $statusKey ? 'selected' : '' }}>
                                {{ $statusLabel }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="flex items-center">
                <input
                    ...
                    name="is_initial"
                    value="1"
                    {{ old('is_initial', $dialog->is_initial) ? 'checked' : '' }}
                    class="mr-2"
                >
        ...
        <div class="bg-white dark:bg-[#161615] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg p-4 sm:p-6 space-y-4">
            ...
            <div id="answers-container">
                @foreach($dialog->answers as $index => $answer)
                    <div class="border border-[#e3e3e0] dark:border-[#3E3E3A] rounded p-4 space-y-4" id="answer-row-{{ $index }}">
                        <div class="flex justify-between items-center">
                            <h3 class="font-medium">Ответ #{{ $index + 1 }}</h3>
                            <button type="button" onclick="removeAnswerRow({{ $index }})" class="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700 text-sm">Удалить</button>
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Текст ответа *</label>
                            <input type="text" name="answers[{{ $index }}][text]" value="{{ old("answers.$index.text", $answer->text) }}" required class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium mb-1">Следующий диалог</label>
                                <select name="answers[{{ $index }}][next_dialog_id]" class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">
                                    <option value="">Нет следующего диалога</option>
                                    @foreach($dialogs as $nextDialog)
                                        <option value="{{ $nextDialog->id }}" {{ old("answers.$index.next_dialog_id", $answer->next_dialog_id) == $nextDialog->id ? 'selected' : '' }}>
                                            {{ Str::limit($nextDialog->text, 50) }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="answers[{{ $index }}][quest_trigger_id]" class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">
                                    <option value="">Не запускать квест</option>
                                    @foreach($quests as $quest)
                                        <option value="{{ $quest->id }}" {{ old("answers.$index.quest_trigger_id", $answer->quest_trigger_id) == $quest->id ? 'selected' : '' }}>
                                            {{ $quest->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="answers[{{ $index }}][item_required_id]" class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">
                                    <option value="">Нет требования</option>
                                    @foreach($items as $item)
                                        <option value="{{ $item->id }}" {{ old("answers.$index.item_required_id", $answer->item_required_id) == $item->id ? 'selected' : '' }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <select name="answers[{{ $index }}][skill_required]" class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">
                                        <option value="">Нет требования</option>
                                        @foreach($skills as $skill)
                                            <option value="{{ $skill->id }}" {{ old("answers.$index.skill_required", $answer->skill_required) == $skill->id ? 'selected' : '' }}>
                                                {{ $skill->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="answers[{{ $index }}][skill_level_required]" value="{{ old("answers.$index.skill_level_required", $answer->skill_level_required) }}" min="1" max="100" class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
let answerIndex = {{ $dialog->answers->count() }};
