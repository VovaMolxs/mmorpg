...
@section('title', 'Редактировать настройку спавна')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl sm:text-3xl font-semibold mb-2">Редактировать настройку спавна</h1>
        <a
    ...
    <form method="POST" action="{{ route('admin.item-spawns.update', $spawn) }}" class="space-y-6">
        @csrf
        @method('PUT')
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($items as $item)
                            <option value="{{ $item->id }}" {{ old('item_id', $spawn->item_id) == $item->id ? 'selected' : '' }}>
                                {{ $item->name }} ({{ $item->type }})
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input
                        ...
                        id="min_quantity"
                        name="min_quantity"
                        value="{{ old('min_quantity', $spawn->min_quantity) }}"
                        min="1"
                        max="1000"
                        required
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
                ...
                <div>
                    ...
                    <input
                        ...
                        id="max_quantity"
                        name="max_quantity"
                        value="{{ old('max_quantity', $spawn->max_quantity) }}"
                        min="1"
                        max="1000"
                        required
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
