...
@section('title', 'Создать спавн монстра')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl sm:text-3xl font-semibold mb-2">Создать спавн монстра</h1>
        <a
    ...
    <form method="POST" action="{{ route('admin.monster-spawns.store') }}" class="space-y-6">
        @csrf
                    ...
                    >
                        ...
                        @foreach($monsters as $monster)
                            <option value="{{ $monster->id }}" {{ old('monster_id') == $monster->id ? 'selected' : '' }}>
                                {{ $monster->name }} (Уровень: {{ $monster->level }}, {{ $ranks[$monster->rank] ?? $monster->rank }})
                            </option>
                        @endforeach
                    </select>
                    @error('monster_id')
