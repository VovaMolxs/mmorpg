...
@section('title', 'Редактировать спавн монстра')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl sm:text-3xl font-semibold mb-2">Редактировать спавн монстра</h1>
        <a
    ...
    <form method="POST" action="{{ route('admin.monster-spawns.update', $spawn) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-[#161615] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg p-4 sm:p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label for="monster_id" class="block text-sm font-medium mb-1">Монстр *</label>
                    <select
                        id="monster_id"
                        name="monster_id"
                        required
                    ...
                    >
                        ...
                        @foreach($monsters as $monster)
                            <option value="{{ $monster->id }}" {{ old('monster_id', $spawn->monster_id) == $monster->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($locations as $location)
                            <option value="{{ $location->id }}" {{ old('location_id', $spawn->location_id) == $location->id ? 'selected' : '' }}>
                                {{ $location->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input
                        ...
                        id="min_instances"
                        name="min_instances"
                        value="{{ old('min_instances', $spawn->min_instances) }}"
                        required
                        min="0"
                        max="100"
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
                ...
                <div>
                    ...
                    <input
                        ...
                        id="max_instances"
                        name="max_instances"
                        value="{{ old('max_instances', $spawn->max_instances) }}"
                        required
                        min="1"
                        max="100"
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('respawn_time_min', $spawn->respawn_time_min) }}"
                        required
                        min="1"
                        max="1440"
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
                ...
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('respawn_time_max', $spawn->respawn_time_max) }}"
                        required
                        min="1"
                        max="1440"
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input
                        ...
                        id="spawn_chance"
                        name="spawn_chance"
                        value="{{ old('spawn_chance', $spawn->spawn_chance) }}"
                        required
                        min="1"
                        max="100"
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
                ...
                <div>
                    ...
                    <input
                        ...
                        id="spawn_radius"
                        name="spawn_radius"
                        value="{{ old('spawn_radius', $spawn->spawn_radius) }}"
                        min="0"
                        max="1000"
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
                    >
        ...
        <div class="bg-white dark:bg-[#161615] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg p-4 sm:p-6 space-y-4">
            ...
            <div>
                ...
                >
                    ...
                    <option value="always" {{ old('spawn_schedule.type', $spawn->spawn_schedule['type'] ?? '') === 'always' ? 'selected' : '' }}>Всегда</option>
                    <option value="day" {{ old('spawn_schedule.type', $spawn->spawn_schedule['type'] ?? '') === 'day' ? 'selected' : '' }}>Только день (6:00 - 22:00)</option>
                    <option value="night" {{ old('spawn_schedule.type', $spawn->spawn_schedule['type'] ?? '') === 'night' ? 'selected' : '' }}>Только ночь (22:00 - 6:00)</option>
                    <option value="custom" {{ old('spawn_schedule.type', $spawn->spawn_schedule['type'] ?? '') === 'custom' ? 'selected' : '' }}>Кастомное</option>
                </select>
            </div>
                    ...
                    @for($i = 0; $i < 24; $i++)
                        <label class="flex items-center">
                            <input
                                ...
                                value="{{ $i }}"
                                {{ in_array($i, old('spawn_schedule.hours', $spawn->spawn_schedule['hours'] ?? [])) ? 'checked' : '' }}
                                class="mr-1"
                            >
