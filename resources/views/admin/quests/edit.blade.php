...
@section('title', 'Редактировать квест')

@section('content')
<div class="max-w-6xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl sm:text-3xl font-semibold mb-2">Редактировать квест: {{ $quest->name }}</h1>
        <a
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        id="max_level"
                        name="max_level"
                        value="{{ old('max_level', $quest->max_level) }}"
                        min="1"
                        max="100"
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
                    >
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    >
                        ...
                        @foreach($npcs as $npc)
                            <option value="{{ $npc->id }}" {{ old('quest_giver_npc_id', $quest->quest_giver_npc_id) == $npc->id ? 'selected' : '' }}>
                                {{ $npc->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    >
                        <option value="">Нет NPC</option>
                        @foreach($npcs as $npc)
                            <option value="{{ $npc->id }}" {{ old('turn_in_npc_id', $quest->turn_in_npc_id) == $npc->id ? 'selected' : '' }}>
                                {{ $npc->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    >
                        ...
                        @foreach($quests as $prevQuest)
                            <option value="{{ $prevQuest->id }}" {{ old('previous_quest_id', $quest->previous_quest_id) == $prevQuest->id ? 'selected' : '' }}>
                                {{ $prevQuest->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('reputation_required', $quest->reputation_required) }}"
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
                    >
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-[#161615] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg p-4 sm:p-6 space-y-4">
            ...
            <div id="objectives-container">
                @foreach($quest->objectives as $index => $objective)
                    <div class="border border-[#e3e3e0] dark:border-[#3E3E3A] rounded p-4 space-y-4" id="objective-row-{{ $index }}">
                        <input type="hidden" name="objectives[{{ $index }}][id]" value="{{ $objective->id }}">
                        <div class="flex justify-between items-center">
                            <h3 class="font-medium">Цель #{{ $index + 1 }}</h3>
                            <button type="button" onclick="removeObjectiveRow({{ $index }})" class="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700 text-sm">Удалить</button>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium mb-1">Тип цели *</label>
                                <select name="objectives[{{ $index }}][type]" required class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">
                                    @foreach($objectiveTypes as $typeKey => $typeLabel)
                                        <option value="{{ $typeKey }}" {{ old("objectives.$index.type", $objective->type) === $typeKey ? 'selected' : '' }}>
                                            {{ $typeLabel }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="number" name="objectives[{{ $index }}][required_count]" value="{{ old("objectives.$index.required_count", $objective->required_count) }}" min="1" required class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">
                            </div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium mb-1">ID цели</label>
                                <input type="number" name="objectives[{{ $index }}][target_id]" value="{{ old("objectives.$index.target_id", $objective->target_id) }}" class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">
                            </div>
                            <div>
                                <label class="block text-sm font-medium mb-1">Название цели</label>
                                <input type="text" name="objectives[{{ $index }}][target_name]" value="{{ old("objectives.$index.target_name", $objective->target_name) }}" class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Описание</label>
                            <textarea name="objectives[{{ $index }}][description]" rows="2" class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">{{ old("objectives.$index.description", $objective->description) }}</textarea>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <div class="bg-white dark:bg-[#161615] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg p-4 sm:p-6 space-y-4">
            ...
            <div id="rewards-container">
                @foreach($quest->rewards as $index => $reward)
                    <div class="border border-[#e3e3e0] dark:border-[#3E3E3A] rounded p-4 space-y-4" id="reward-row-{{ $index }}">
                        <input type="hidden" name="rewards[{{ $index }}][id]" value="{{ $reward->id }}">
                        <div class="flex justify-between items-center">
                            <h3 class="font-medium">Награда #{{ $index + 1 }}</h3>
                            <button type="button" onclick="removeRewardRow({{ $index }})" class="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700 text-sm">Удалить</button>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium mb-1">Тип награды *</label>
                                <select name="rewards[{{ $index }}][type]" required class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]" onchange="updateRewardFields({{ $index }})">
                                    @foreach($rewardTypes as $typeKey => $typeLabel)
                                        <option value="{{ $typeKey }}" {{ old("rewards.$index.type", $reward->type) === $typeKey ? 'selected' : '' }}>
                                            {{ $typeLabel }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="number" name="rewards[{{ $index }}][quantity]" value="{{ old("rewards.$index.quantity", $reward->quantity) }}" min="1" required class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">
                            </div>
                        </div>
                        <div id="reward-{{ $index }}-item" style="display: {{ old("rewards.$index.type", $reward->type) === 'item' ? 'block' : 'none' }};">
                            <label class="block text-sm font-medium mb-1">Предмет</label>
                            <select name="rewards[{{ $index }}][reward_id]" class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">
                                <option value="">Выберите предмет</option>
                                @foreach($items as $item)
                                    <option value="{{ $item->id }}" {{ old("rewards.$index.reward_id", $reward->reward_id) == $item->id ? 'selected' : '' }}>
                                        {{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div id="reward-{{ $index }}-skill" style="display: {{ old("rewards.$index.type", $reward->type) === 'skill' ? 'block' : 'none' }};">
                            <label class="block text-sm font-medium mb-1">Навык</label>
                            <select name="rewards[{{ $index }}][reward_id]" class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a]">
                                <option value="">Выберите навык</option>
                                @foreach($skills as $skill)
                                    <option value="{{ $skill->id }}" {{ old("rewards.$index.reward_id", $reward->reward_id) == $skill->id ? 'selected' : '' }}>
                                        {{ $skill->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" id="rewards[{{ $index }}][is_choice]" name="rewards[{{ $index }}][is_choice]" value="1" {{ old("rewards.$index.is_choice", $reward->is_choice) ? 'checked' : '' }} class="mr-2">
                            <label for="rewards[{{ $index }}][is_choice]" class="text-sm">Награда на выбор</label>
                        </div>
                    </div>
                @endforeach
            </div>
...
<script>
let objectiveIndex = {{ $quest->objectives->count() }};
let rewardIndex = {{ $quest->rewards->count() }};
