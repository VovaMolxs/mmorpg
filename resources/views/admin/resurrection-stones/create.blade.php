...
@section('title', 'Создать камень воскрешения')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl sm:text-3xl font-semibold mb-2">Создать камень воскрешения</h1>
        <a
    ...
    <form method="POST" action="{{ route('admin.resurrection-stones.store') }}" class="space-y-6">
        @csrf
        ...
        <div class="bg-white dark:bg-[#161615] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg p-4 sm:p-6 space-y-4">
            ...
            <div>
                ...
                <input
                    ...
                    required
                    maxlength="255"
                    placeholder="Например: Алтарь возрождения"
                    class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
                >
                @error('name')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            ...
            <div>
                ...
                <textarea
                    ...
                    name="description"
                    rows="3"
                    placeholder="Описание камня и его внешнего вида..."
                    class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('cooldown_minutes', 0) }}"
                        min="0"
                        max="1440"
                        required
                        placeholder="0 = без перезарядки"
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
                    >
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select
                        ...
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded bg-white dark:bg-[#0a0a0a] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433]"
                    >
                        <option value="glow" {{ old('visual_effect', 'glow') == 'glow' ? 'selected' : '' }}>Свечение (glow)</option>
                        <option value="particles" {{ old('visual_effect') == 'particles' ? 'selected' : '' }}>Частицы (particles)</option>
                        <option value="aura" {{ old('visual_effect') == 'aura' ? 'selected' : '' }}>Аура (aura)</option>
                    </select>
                    @error('visual_effect')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
